            ...
            <div className="mb-8 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {groupByCategory().map((category) => {
                  const catKey = category.title.toLowerCase().includes("dakuten")
                    ? "tenten"
                    : category.title.toLowerCase().includes("handakuten")
                    ? "maru"
                    : category.title.toLowerCase().includes("combination")
                    ? "combinations"
                    : "basic";
                  ...
                  return (
                    <div
                      key={category.title}
                      className={`p-3 rounded-xl border-2 cursor-pointer hover:shadow-md transition-all ${filterType === catKey ? "ring-2 ring-blue-500" : ""} ${getCategoryColor(catKey as any)}`}
                      onClick={() => setFilterType(catKey as any)}
                    >
